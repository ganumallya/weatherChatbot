...
app.post('/modem',function(req,res){
	...
	if(mVaction == 'iDirect'){
		mVtype = req.body.result.parameters.devicetype;
			...
			speech = "<table><tr><td><b>Model No.</b></td><td>	<b>Name</b></td></tr>";
			for(var i=0;i<tempType.length;i++){
				speech = speech +'<tr><td>'+ tempType[i].modelNo +"</td><td>"+tempType[i].name+"</td><tr>";
			}
			speech = speech + '</table>';
			tempContext = {
				"name":"contextone",
			...
			finalResponse = {
					"speech":speech + ",, Please enter the <b>Model No. </b> from the list, for which you would like to know more about.",
					"displayText":speech+",, Please enter the<b> Model No. </b> from the list, for which you would like to know more about.",
					"contextOut":[tempContext]
			}
			res.send(finalResponse);
